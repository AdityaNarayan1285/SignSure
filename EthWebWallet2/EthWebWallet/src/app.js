...
const ALCHEMY_API_KEY = "H5RcUwjRrnaEMwATtSp4L";
...
const App = {
  ...
  setupWallet(wallet) {
    showWallet();
    $('#wallet-submit-refresh').off().on('click', async () => {
  await App.refreshUI();
  await App.loadTxHistory();
});
...
async loadTxHistory() {
  ...
  const address = App.activeWallet.address.toLowerCase();
  ...
  try {
    ...
    // 🔥 FETCH SENT TXs
    const sent = await fetchTransfers({
      fromBlock: "0x0",
      toBlock: "latest",
      fromAddress: address,
      category: ["external"],
    ...
    // 🔥 FETCH RECEIVED TXs
    const received = await fetchTransfers({
      fromBlock: "0x0",
      toBlock: "latest",
      toAddress: address,
      category: ["external"],
    ...
    const all = [...sent, ...received]
      .filter((tx, i, arr) =>
        arr.findIndex(t => t.hash === tx.hash) === i
      )
      .sort(
        (a, b) =>
      ...
      .slice(0, 10); // show latest 20

    box.innerHTML = "";

    if (all.length === 0) {
      box.innerHTML = "<i>No transactions found</i>";
      return;
    }
    ...
    all.forEach(tx => {
      ...
      div.innerHTML = `
        ...
        Hash: ${tx.hash}<br>
        To: ${tx.to}<br>
        Amount: ${tx.value || "0"} ETH<br>
        Time: ${new Date(tx.metadata.blockTimestamp).toLocaleString()}
      `;
    ...
    console.error("Alchemy history error:", err);
    box.innerHTML = "<i>Error loading history</i>";
  }
}
,
